...
# === Возможные причины для репорта пользователя на Scratch ===
...
# Основные списки аккаунтов, которые будут использоваться программой
accounts = []
passwords = []
...
def spam(tar, text):
	...
	while True:
		try:
			...
			print(f'{datetime.datetime.now().time()} Сообщение "{text}" отправлено к {tar}')
			time.sleep(random.uniform(1.5, 3.5))

			if keyboard.is_pressed(' '):  # если пользователь зажал пробел — остановка
				print('Операция прервана по запросу пользователя.')
				break
		except Exception as e:
			print(f"Ошибка: {e}")
			break

# === Автоматическая жалоба на пользователя ===
def reportall(tar):
	while True:
		...
		try:
			...
			if keyboard.is_pressed(' '):  # остановка по пробелу
				print('Операция прервана по запросу пользователя.')
				break
...
def main():
	while True:
		print("\nЧто-бы прервать любую операцию (кроме ввода), зажмите пробел")
